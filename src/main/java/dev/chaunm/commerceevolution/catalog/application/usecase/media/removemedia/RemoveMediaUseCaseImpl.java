...
public class RemoveMediaUseCaseImpl implements RemoveMediaUseCase {
    ...
    @Override
    @Transactional
    public void removeMedia(RemoveMediaCommand command) {
        Product product = productRepository.findById(new ProductId(command.productId()))
                .orElseThrow(ProductNotFoundException::new);

        product.removeMedia(new MediaId(command.mediaId()));
